...
const ReteSolutionSection = () => {
  ...
  return (
    <section ref={ref as React.RefObject<HTMLElement>} id="vantaggi" className="py-16 md:py-24 bg-background">
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className={`bg-gradient-to-br from-primary/10 via-gold/5 to-primary/10 border border-primary/20 rounded-2xl p-5 md:p-12 mb-10 md:mb-12 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '200ms' }}>
            <p className="text-xl text-foreground mb-6">
              <strong className="text-gold">RF Ristrutturazioni Debiti</strong> è la <strong>PRIMA rete nazionale</strong> in Italia che unisce Avvocati, Commercialisti e Consulenti del Lavoro specializzati esclusivamente in:
            </p>
          ...
          {/* What You Get */}
          <div className={`mb-10 md:mb-12 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '400ms' }}>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {benefits.map((benefit, index) => (
                <div 
                  key={index} 
                  className={`flex items-start gap-3 bg-card border border-border rounded-lg p-3 md:p-4 hover:border-primary/50 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
          ...
          {/* Results */}
          <div className={`bg-navy rounded-2xl p-5 md:p-12 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '800ms' }}>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {results.map((result, index) => (
                <div 
                  key={index} 
                  className={`flex items-start gap-3 bg-white/5 border border-white/10 rounded-lg p-3 md:p-4 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
