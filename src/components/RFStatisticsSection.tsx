      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-16">
            <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-6 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              I Numeri Che Contano
            </h2>
            <p className={`text-xl text-muted-foreground transition-all duration-700 delay-100 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          {/* Bottom explanation */}
          <div className={`mt-16 p-8 rounded-3xl bg-background border border-border shadow-soft text-center transition-all duration-700 delay-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <p className="text-lg mt-6 text-primary font-semibold">
...
const StatCard = ({ stat, index, isInView }: StatCardProps) => {
  ...
  return (
    <div
      className={`p-6 md:p-8 rounded-2xl bg-background border-2 ${colors.border} transition-all duration-500 shadow-card hover:shadow-soft ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
      style={{ transitionDelay: `${200 + index * 100}ms` }}
    >
      <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl ${colors.bg} flex items-center justify-center mb-4`}>
        <stat.icon className={`w-6 h-6 md:w-7 md:h-7 ${colors.text}`} />
      </div>
