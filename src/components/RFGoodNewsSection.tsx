      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-8 md:mb-16">
            ...
            <h2 className={`text-2xl md:text-4xl lg:text-5xl font-bold text-foreground leading-tight mb-4 md:mb-6 transition-all duration-700 delay-100 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              Anzi, Un'<span className="text-primary">Ottima</span> Notizia
            </h2>
            <p className={`text-base md:text-xl text-muted-foreground max-w-2xl mx-auto transition-all duration-700 delay-200 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-8 md:mb-16">
            {benefits.map((benefit, index) => (
              <div
                key={index}
                className={`group p-4 md:p-6 rounded-xl md:rounded-2xl bg-background border-2 border-primary/20 hover:border-primary transition-all duration-500 shadow-card hover:shadow-primary ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${300 + index * 100}ms` }}
              >
                <div className="flex items-start gap-3 md:gap-4">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-gold/10 group-hover:bg-gold flex items-center justify-center flex-shrink-0 transition-colors duration-300">
                    <benefit.icon className="w-5 h-5 md:w-6 md:h-6 text-gold group-hover:text-gold-foreground transition-colors duration-300" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <Check className={`w-4 h-4 md:w-5 md:h-5 text-gold transition-all duration-500 ${isInView ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`} style={{ transitionDelay: `${600 + index * 100}ms` }} />
          ...
          {/* Caveat */}
          <div className={`text-center p-5 md:p-8 rounded-2xl md:rounded-3xl bg-primary/10 border border-primary/20 transition-all duration-700 delay-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <ul className="text-sm md:text-base text-muted-foreground space-y-1.5 md:space-y-2 max-w-lg mx-auto text-left md:text-center">
            ...
            <p className="text-lg md:text-xl font-bold text-primary mt-4 md:mt-6">
